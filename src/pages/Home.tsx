	...
	useEffect(() => {
		dispatch(getPeople()).then(result => {
			if (getPeople.rejected.match(result)) {
				dispatch(
					sendMessage({ message: 'Ocorreu um erro ao carregar os dados', messageType: MessageType.Error })
				);
			}
		});
	}, [dispatch]);
	...
	return (
		<>
			...
			<div className="create-new">
				<button type="button" onClick={() => goToNewPage()}>
					Criar novo cadastro
				</button>
			</div>
				...
				<tbody>
					{peopleData.map((person, i) => (
						<tr key={i}>
							...
							<td>
								<button type="button" onClick={() => goToEditPage(person.id)}>
									Editar
								</button>
							</td>
						</tr>
